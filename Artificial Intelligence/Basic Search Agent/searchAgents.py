...
class CornersProblem(SearchProblem):
    ...
    def isGoal(self, state):
        # visualizing
        # print("----------isGoal------------")
        # print("State 0 is", end = "")
        # print(state[0])
        # print("State 1 is", end = "")
        # print(state[1])
        current = state[0]
        visitedCorners = state[1]

        # if the current position is an unvisited corner position, append it to visited corners
        if current in self.corners:
            if current not in visitedCorners:
                visitedCorners.append(current)
            return len(visitedCorners) == 4     # returns true if all corners are visited
        return False
    ...
    def successorStates(self, state):
        ...
        for action in Directions.CARDINAL:
            ...
            # if the next position is not a wall, append the next position to successors
            if (not hitsWall):
                successorVC = list(state[1])
                nextPos = (nextx, nexty)
                # print("Next Position:", end = "")
                # print(nextPos)
                # if the next position is a corner and not in visited corners, append it
                if nextPos in self.corners and nextPos not in successorVC:
                    successorVC.append(nextPos)
                successor = ((nextPos, successorVC), action, 1)
                # appends the next position to the successor
...
def cornersHeuristic(state, problem):
    ...
    # *** Your Code Here ***
    # visualizing code
    # print("----------cornerHeuristic------------")
    # print("Corners is: ", end = "")
    ...
    # finds corners that are left to reach the goal state
    visited = state[1]
    cornersLeft = []
    for corner in problem.corners:
        if corner not in visited:
            cornersLeft.append(corner)
    # print("Corners left: ", end = "")
    # print(cornersLeft)

    # find heuristic for unvisited corners
    total = 0
    while cornersLeft:              # loops through all unvisited corners
        for corner in cornersLeft:  # and finds the distance to the closest corner
            dist, corner = min([(distance.manhattan(state[0], corner), corner)])
        total += dist
        cornersLeft.remove(corner)
    # so we don't want an exact value since that's not admissable
    # print("Total is: ", end = "")
    # print(total)
    # print("Heuristic total is: ", end = "")
    # print(int(total*3/4))
...
def foodHeuristic(state, problem):
    ...
    # returns the distance from the current position to the furthest food
    dist = 0
    for food in foodList:
        # print("Food is: ", end = "")
        # print(food)
        # print("Maze distance is: ", end = "")
        # print(distance.maze(state[0], food, problem.startingGameState))
        dist2 = distance.maze(state[0], food, problem.startingGameState)
        dist = max(dist2, dist)
    # print("Max distance is: ", end = "")
    # print(dist)
    return dist
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        # *** Your Code Here ***
        # calls bfs since that's basically what we need
